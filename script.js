  ...
  */
  ...
  function draw(callbackTime) {
    ...
    for (let j = 0; j < itemArray.length; j++) {
      ...
      // move the item
      item.moveToPoint(refreshThrottle);
    }
  ...
  class Item {
    constructor(x, y, size) {
      ...
      this.moveToPoint = function (refreshThrottle) {
        if (this.x == this.ptx && this.y == this.pty) {
          this.pickNewPoint();
        }
        if (Math.abs(this.x - this.ptx) <= this.speed * refreshThrottle) {
          this.x = this.ptx;
        } else {
          this.x > this.ptx ? this.x -= this.speed * refreshThrottle : this.x += this.speed * refreshThrottle;
        }
        if (Math.abs(this.y - this.pty) <= this.speed * refreshThrottle) {
          this.y = this.pty;
        } else {
          this.y > this.pty ? this.y -= this.speed * refreshThrottle : this.y += this.speed * refreshThrottle;
        }
        this.updateBBox();
      };
